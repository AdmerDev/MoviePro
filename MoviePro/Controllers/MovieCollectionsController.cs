...
namespace MoviePro.Controllers
{
    public class MovieCollectionsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? id)
        {
            ...
            var movieIdsInCollections = await _context.MovieCollection
                                         .Where(m => m.CollectionId == id)
                                         .OrderBy(m => m.Order)
                                         .Select(m => m.MovieId)
                                         .ToListAsync();

            var movieIdsNotInCollection = allMovieIds.Except(movieIdsInCollections);

            var moviesInCollection = new List<Movie>();
            movieIdsInCollections.ForEach(movieId => moviesInCollection.Add(_context.Movie.Find(movieId)));
